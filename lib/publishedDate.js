...
// Posts older than this stop accepting new votes/comments (still fully
// visible, just read-only) - see isArchived below. Analysis pieces are
// longer-form and worth discussing for longer than a News clip, hence the
// longer window. This is a UI-only gate, not enforced by Supabase RLS:
// news_post_votes/news_post_comments have no way to know a post's
// Prismic-sourced publish date, so a determined user could still call the
// Supabase API directly to vote on an archived post. Worth revisiting with
// a trusted server-side check (e.g. storing each post's published_at in
// Supabase) if that turns out to matter in practice - low stakes for now,
// unlike the comment depth cap and flood guard in supabase/migrations,
// which guard against real abuse.
...
export function isArchived(doc, postType = 'news') {
  return isOlderThanDays(effectivePublishedAt(doc), ARCHIVE_AFTER_DAYS[postType]);
}
