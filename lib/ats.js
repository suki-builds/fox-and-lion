...
export const COMPANIES = [
  ...
  // same integration pattern as the three above.
...
// had to (Anduril's list alone went from ~2MB to ~49MB with content
// included; that concern doesn't apply to a once-a-day background job).
...
// page (getAtsJobDetail below) needs. Anduril's descriptions alone would add
// several MB across 2000+ rows for data the list never displays.
...
// the hard way: Anduril alone has 2000+ open roles, so a plain
// `.select(...)` silently truncated the table to its first 1000 rows the
// moment the total crossed that line, with no error to signal it. Pages
// through with .range() until a page comes back short of a full page.
