...
// no scraping. The raw bulk-list responses are too large for Next's fetch
// cache (Anduril's alone is ~2.3MB, Palantir's Lever list ~7.6MB with
// descriptions baked in — over the 2MB per-item cache limit either way),
// so the individual `fetchJson` calls below use `cache: 'no-store'` and
// we instead cache the small *normalized* result of `getAllJobs` itself
// via `unstable_cache` — see bottom of this file. That result is a few
// hundred KB at most, comfortably cacheable, and is what actually governs
// how often /jobs and the homepage re-hit the origin APIs.
//
...
// Caching is intentionally punted for the bulk list (see module comment).
