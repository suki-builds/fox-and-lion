...
// no scraping, no caching or scheduled refresh yet for the list itself.
// Every request to /jobs re-fetches all APIs live (see
// `dynamic = 'force-dynamic'` in app/(public)/jobs/page.js). Individual
// job-detail pages (app/(public)/jobs/[company]/[id]/page.js) fetch a
// single small posting each, so those *are* cached (see `revalidate`
// there) — this is fine as a first pass; revisit once there's a real
// refresh/caching strategy for the bulk list.
//
