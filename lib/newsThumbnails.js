...
import { READ_ATTEMPTS, isBuildPhase, withRetry } from './resilientRead';
...
// Reads stored thumbnails, retrying transient failures (see
// lib/resilientRead.js). Throws if every attempt fails.
//
...
export async function getBatchedThumbnails(uidToSourceUrl) {
  ...
  // stored" - see lib/resilientRead.js. At runtime that throws, so the last
  // good page keeps being served. During a build it degrades to no
  // thumbnails, since that is cosmetic and corrects itself at the page's
  // next regeneration, and isn't worth failing a deploy over.
  let rows;
  try {
