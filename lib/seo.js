...
export function buildMetadata({ data, fallbackTitle, fallbackDescription, fallbackImage }) {
  ...
  // all. cover_image is displayed on-page at its native aspect ratio (see
  // lib/prismicImage.js's coverImageSrc), which usually isn't Open
  // Graph's expected 1200x630, so it's run through socialImageSrc()'s
  // fixed crop here instead. fallbackImage is left untouched instead - it's an
