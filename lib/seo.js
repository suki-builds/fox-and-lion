...
// Kept in step with app/layout.js - see the note on DEFAULT_SOCIAL_IMAGE there.
...
export function buildMetadata({ data, fallbackTitle, fallbackDescription, fallbackImage }) {
  ...
  if (isFilled.image(data?.meta_image)) {
    ...
    // Nothing document-specific resolved - a News post whose source-article
    // scrape came back empty, say. Returning no image here doesn't mean
    // "no preview image", it means the scraper picks one out of the page
    // itself, and the first thing it finds is the ad banner. So always ship
    // something deliberate.
    imageUrl = DEFAULT_SOCIAL_IMAGE;
    imageWidth = 1200;
    imageHeight = 630;
    imageAlt = 'Fox and Lion';
  }
