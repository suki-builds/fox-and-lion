...
let contactMessageItemTypeId = null;

async function getContactMessageItemTypeId(token) {
  if (contactMessageItemTypeId) return contactMessageItemTypeId;

  const { data } = await cmaRequest('/item-types', { token });
  const itemType = data.find((it) => it.attributes.api_key === 'contact_message');
  if (!itemType) {
    throw new Error('DatoCMS: no item type found with api_key "contact_message"');
  }
  contactMessageItemTypeId = itemType.id;
  return contactMessageItemTypeId;
}

// Creates a Contact Message record. replied is deliberately omitted from
// attributes so the field's own default (false) applies.
export async function createContactMessage({
  ...
  const token = process.env.DATOCMS_CONTACT_FORM_TOKEN;
  if (!token) {
    throw new Error('DATOCMS_CONTACT_FORM_TOKEN is not set');
  }

  const itemTypeId = await getContactMessageItemTypeId(token);
