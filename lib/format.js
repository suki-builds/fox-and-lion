...
// Prefers the outlet's real, self-declared name (og:site_name, fetched and
// cached via lib/ogImage.js) over guessing one from the domain — e.g. a
// site's og:site_name says "The Record" outright, where the domain-guessing
// fallback below would mangle "therecord.media" into "Therecord" unless
// manually added to the override list.
export function resolveSourceName(siteName, url) {
  return (siteName && siteName.trim()) || sourceNameFromUrl(url);
}
